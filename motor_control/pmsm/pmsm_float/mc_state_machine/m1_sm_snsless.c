...
RAM_FUNC_LIB
static void M1_TransRunCalibSpin(void);
RAM_FUNC_LIB
...
/*!
 * @brief Transition from Calib to Spin state
 *
 * @param void  No input parameter
 *
 * @return None
...
{
	float_t tempflt,tempfltabs;

	tempflt = fltSpeedCmd / g_fltM1speedAngularScale;

	tempfltabs = MLIB_Abs_FLT(tempflt);

    if (g_bM1SwitchAppOnOff)
    {
        /* Set speed */
        if ( tempfltabs < g_sM1Drive.sStartUp.fltSpeedCatchUp)
        {
            g_sM1Drive.sSpeed.fltSpeedCmd = 0.0F;
        }
        else if (tempfltabs > M1_N_OVERSPEED)
        {
            g_sM1Drive.sSpeed.fltSpeedCmd = 0.0F;
        }
        else
        {
            g_sM1Drive.sSpeed.fltSpeedCmd = tempflt;
        }
    }
    else
    {
